...
import { classifyBrowserMethod, DAPP_BOOKMARKS, normalizeDappUrl } from '../wallet/dapps';
import {
  chainIdHex,
...
export function BrowserScreen() {
  ...
  const [draft, setDraft] = useState(DAPP_BOOKMARKS[0]!.url);
  const [url, setUrl] = useState(DAPP_BOOKMARKS[0]!.url);
  ...
  return (
    <View style={styles.root}>
      ...
      <WebView
        ...
        onError={(event) => setError(event.nativeEvent.description || 'Page failed to load.')}
      />
    </View>
  );
}
